...
def _generate_readings(num_readings_gen, time_between, nan_timestamp=False):
    ...
    for i in range(num_readings_gen):
        ...
        channel = CHANNEL_NUMBERS[
            i % CHANNEL_COUNT
        ]  # assign each generated reading to channel (read mod no of chnls)
...
class SetUpTests(unittest.TestCase):
    ...
    def test_WHEN_scan_state_set_THEN_scan_state_matches_the_set_state(self):
        ...
        for enum_value, string_value in sample_data.items():
            self.ca.assert_setting_setpoint_sets_readback(
                enum_value, "SCAN:STATE", expected_value=string_value
            )
    ...
    def test_WHEN_delay_state_set_THEN_delay_state_matches_the_set_state(self):
        for enum_value, string_value in on_off_status.items():
            self.ca.assert_setting_setpoint_sets_readback(
                enum_value, "DELAYMODE", expected_value=string_value
            )
    ...
    def test_WHEN_source_set_THEN_source_matches_the_set_state(self):
        ...
        for enum_value, string_value in sample_data.items():
            self.ca.assert_setting_setpoint_sets_readback(
                enum_value, "CONTROLSOURCE", expected_value=string_value
            )
    ...
    def test_WHEN_buffer_feed_set_THEN_buffer_feed_matches_the_set_state(self):
        ...
        for enum_value, string_value in sample_data.items():
            self.ca.assert_setting_setpoint_sets_readback(
                enum_value, "BUFF:SOURCE", expected_value=string_value
            )

    def test_WHEN_init_state_set_THEN_init_state_matches_the_set_state(self):
        for enum_value, string_value in on_off_status.items():
            self.ca.assert_setting_setpoint_sets_readback(
                enum_value, "INITMODE", expected_value=string_value
            )
    ...
    def test_WHEN_buffer_control_set_THEN_buffer_control_matches_the_set_state(self):
        ...
        for enum_value, string_value in sample_data.items():
            self.ca.assert_setting_setpoint_sets_readback(
                enum_value, "BUFF:CONTROLMODE", expected_value=string_value
            )

    def test_WHEN_sample_count_set_THEN_sample_count_matches_the_set_state_AND_within_alarm_is_major(
        self,
    ):
        expected_alarm = "MAJOR"
        sample_data = [0, 70000]
        for set_value in sample_data:
            self.ca.assert_setting_setpoint_sets_readback(
                set_value, "SAMPLECOUNT", expected_alarm=expected_alarm
            )

    def test_WHEN_sample_count_set_THEN_sample_count_matches_the_set_state_AND_within_alarm_is_none(
        self,
    ):
        expected_alarm = "NO_ALARM"
        sample_data = [2, 55000]
        for enum_value in sample_data:
            self.ca.assert_setting_setpoint_sets_readback(
                enum_value, "SAMPLECOUNT", expected_value=enum_value, expected_alarm=expected_alarm
            )

    def test_WHEN_cycles_rate_set_THEN_cycle_rate_matches_the_set_state_AND_within_alarm_is_major(
        self,
    ):
        expected_alarm = "MAJOR"
        sample_data = [0, 65.0]
        for set_value in sample_data:
            self.ca.assert_setting_setpoint_sets_readback(
                set_value, "FRES:NPLC", expected_alarm=expected_alarm
            )

    def test_WHEN_cycles_rate_set_THEN_cycle_rate_matches_the_set_state_AND_within_alarm_is_none(
        self,
    ):
        expected_alarm = "NO_ALARM"
        sample_data = [0.1, 2.0]
        for enum_value in sample_data:
            self.ca.assert_setting_setpoint_sets_readback(
                enum_value, "FRES:NPLC", expected_value=enum_value, expected_alarm=expected_alarm
            )
    ...
    def test_WHEN_auto_range_set_THEN_auto_range_matches_the_set_state(self):
        for enum_value, expected_state in on_off_status.items():
            self.ca.assert_setting_setpoint_sets_readback(
                enum_value, "FRES:AUTORANGE", expected_value=expected_state
            )
    ...
    def test_WHEN_time_stamp_set_to_absolute_THEN_time_stamp_matches_the_set_state(self):
        ...
        for enum_value, expected_state in sample_data.items():
            self.ca.assert_setting_setpoint_sets_readback(
                enum_value, "TIME:FORMAT", expected_value=expected_state
            )
    ...
    def test_WHEN_fres_digits_set_THEN_fres_digits_matches_the_set_state_AND_alarm_is_major(self):
        ...
        for sample_value in sample_data:
            self.ca.assert_setting_setpoint_sets_readback(
                sample_value, "FRES:DIGITS", expected_alarm=expected_alarm
            )
...
class ChannelTests(unittest.TestCase):
    ...
    def test_GIVEN_empty_buffer_WHEN_reading_inserted_THEN_channel_PVs_get_correct_values(self):
        ...
        self.ca.assert_that_pv_is_number(
            "CHNL:101:READ", expected_values["read"], tolerance=READ_TOLERANCE
        )
        self.ca.assert_that_pv_is_number(
            "CHNL:101:TIME", expected_values["time"], tolerance=TIME_TOLERANCE
        )
        self.ca.assert_that_pv_is_number(
            "CHNL:101:TEMP", expected_values["temp101"], tolerance=TEMP_TOLERANCE
        )
        self.ca.assert_that_pv_is_number(
            "CHNL:101:DRIFT", expected_values["drift"], tolerance=DRIFT_TOLERANCE
        )
...
class DriftTests(unittest.TestCase):
    ...
    def test_GIVEN_empty_buffer_WHEN_values_added_THEN_temp_AND_drift_correct(
        ...
        for i in range(0, len(test_data)):
            ...
            # THEN
            self.ca.assert_that_pv_is_number(
                "CHNL:101:DRIFT", test_data[i][2], tolerance=DRIFT_TOLERANCE
            )
            self.ca.assert_that_pv_is_number(
                "CHNL:101:TEMP", test_data[i][1], tolerance=TEMP_TOLERANCE
            )
